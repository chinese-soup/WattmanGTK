...
#for applying user defined configs at runtime
import subprocess #executes the script
from pathlib import Path #manages the path directory
...
class Handler:
    ...
    def apply(self, button):
        #Code modified for implementing the UI apply function by Josh "Hakaru" Cantwell (cantwell.josh@protonmail.com) on 12.9.20 (UK date format)
        # Documentation: https://dri.freedesktop.org/docs/drm/gpu/amdgpu.html

        #sets the filepath of the config file to /var/lib/WattmanCfg so it is user-independent as the program needs to be run as root to talk to the gpu
        path = "/var/lib/WattmanCfg"
